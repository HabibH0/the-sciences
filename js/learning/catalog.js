...
export function completedLessonHtml(state, mod, lesson) {
  ...
  return `<section class="mz-completed"><span class="mz-completed-mark" aria-hidden="true">✓</span><p class="mz-eyebrow">LESSON COMPLETE</p><h1>${escBidi(lesson.title)}</h1><p>${logic ? 'You have worked through the lesson. Review will help you build lasting understanding.' : 'You have passed the quiz. Keep practising to make this knowledge last.'}</p>
    ${score ? `<div class="mz-result-score"><strong>${score.correct} <small>/ ${score.total}</small></strong><span>${logic ? 'correct on the first attempt' : 'correct quiz answers'}</span></div>` : ''}
