...
export const PHASES = ['prepare', 'notice', 'manipulate', 'decode', 'understand', 'clean', 'transfer', 'review', 'assess'];
export const ORDER_TYPES = new Set(['reorder_chunks', 'sentence_reconstruction', 'poetry_reorder']);
export const CHOICE_TYPES = new Set(['contextual_vocabulary', 'choose_meaning', 'pronoun_reference', 'clause_relationship', 'arabic_paraphrase', 'grammatical_function', 'contextual_fill_blank', 'interpretation_choice', 'comprehension', 'transfer_example', 'synonym_contrast', 'collocation_choice', 'root_family', 'rapid_recognition']);
export const EXERCISE_TYPES = new Set([...ORDER_TYPES, ...CHOICE_TYPES, 'chunk_sentence', 'clause_boundary', 'match_pairs', 'typed_recall']);
...
const answerValue = value => typeof value === 'string' ? value.slice(0, 500) : Number.isInteger(value) && value >= 0 && value < 100 ? value
  : Array.isArray(value) ? value.filter(n => Number.isInteger(n) && n >= 0 && n < 100).slice(0, 100) : null;
const clone = value => JSON.parse(JSON.stringify(value));

export function lessonSteps(lesson, run = null) {
  if (lesson.flow) {
    const exercises = new Map(lesson.exercises.map(e => [e.id, e]));
    const flow = lesson.adaptive && run?.selection?.length ? run.selection : lesson.flow;
    return [{ id: 'orientation', phase: 'prepare', type: 'orientation' }, ...flow.map(id => {
      if (id === 'clean') return { id, phase: 'clean', type: 'clean' };
      const exercise = exercises.get(id);
      if (!exercise) throw new Error(`${lesson.id}: unknown activity ${id}`);
      return exercise;
    })];
  }
...
export function emptyLiterature() { return { version: 2, runs: {}, sourceViews: {}, preferences: { stage: null, updatedAt: 0 } }; }
// Opening the archive is exposure, not a successful answer or completion.
export function recordLiteratureSourceView(progress, bookId, chapterId, at) {
  const result = normalizeLiterature(progress);
  if (safeId(bookId) && safeId(chapterId) && timestamp(at)) {
    const key = `${bookId}/${chapterId}`;
    result.sourceViews[key] = Math.min(result.sourceViews[key] || Infinity, timestamp(at));
  }
  return result;
}
export function normalizeLiterature(value) {
  const result = emptyLiterature();
  for (const [key, at] of Object.entries(object(value?.sourceViews) ? value.sourceViews : {})) {
    const parts = key.split('/');
    if (parts.length === 2 && parts.every(safeId) && timestamp(at)) result.sourceViews[key] = timestamp(at);
  }
  if (object(value?.preferences)) result.preferences = { stage: Number.isInteger(value.preferences.stage) && value.preferences.stage >= 1 && value.preferences.stage <= 6 ? value.preferences.stage : null, updatedAt: timestamp(value.preferences.updatedAt) };
  ...
  for (const [id, raw] of Object.entries(value.runs)) {
    ...
    const run = { id, lessonId: raw.lessonId, revision: bounded(raw.revision), startedAt: timestamp(raw.startedAt), updatedAt: timestamp(raw.updatedAt), position: bounded(raw.position, 500), selection: [...new Set((Array.isArray(raw.selection) ? raw.selection : []).filter(safeId))].slice(0, 20), drafts: {}, help: {}, acknowledged: {}, attempts: [] };
    for (const [key, draft] of Object.entries(object(raw.drafts) ? raw.drafts : {})) {
      if (safeId(key) && object(draft)) run.drafts[key] = { answer: answerValue(draft.answer), updatedAt: timestamp(draft.updatedAt), checking: draft.checking === true, revealed: draft.revealed === true, conceptOpen: draft.conceptOpen === true, enteredAt: timestamp(draft.enteredAt) };
    }
    ...
    for (const attempt of Array.isArray(raw.attempts) ? raw.attempts : []) {
      ...
      seen.add(attempt.id);
      run.attempts.push({ id: attempt.id, exerciseId: attempt.exerciseId, at: timestamp(attempt.at), answer: answerValue(attempt.answer), hints: bounded(attempt.hints, 6), support: object(attempt.support) ? Object.fromEntries(Object.entries(attempt.support).filter(([k]) => safeId(k)).map(([k,v]) => [k,bounded(v,6)])) : {}, responseMs: bounded(attempt.responseMs, 86400000) });
    }
...
export function mergeLiterature(local, remote) {
  ...
  const result = emptyLiterature();
  for (const key of new Set([...Object.keys(left.sourceViews), ...Object.keys(right.sourceViews)])) result.sourceViews[key] = Math.min(left.sourceViews[key] || Infinity, right.sourceViews[key] || Infinity);
  result.preferences = clone(left.preferences.updatedAt > right.preferences.updatedAt ? left.preferences : left.preferences.updatedAt < right.preferences.updatedAt ? right.preferences : JSON.stringify(left.preferences) < JSON.stringify(right.preferences) ? left.preferences : right.preferences);
  for (const id of new Set([...Object.keys(left.runs), ...Object.keys(right.runs)])) {
    ...
    if (a.lessonId !== b.lessonId || a.revision !== b.revision || JSON.stringify(a.selection) !== JSON.stringify(b.selection)) { result.runs[id] = clone(JSON.stringify(a) < JSON.stringify(b) ? a : b); continue; }
...
export function createLiteratureRun(lesson, id, now, selection = []) {
  return { id, lessonId: lesson.id, revision: lesson.revision, startedAt: now, updatedAt: now, position: 0, selection: lesson.adaptive ? selection.filter(id => lesson.exercises.some(e => e.id === id)).slice(0,20) : [], drafts: {}, help: {}, acknowledged: {}, attempts: [] };
}
...
export function reconcileLiteratureRun(lesson, run) {
  ...
  for (const [id, draft] of Object.entries(result.drafts)) {
    ...
    if (exercise.type === 'typed_recall') { if (typeof draft.answer !== 'string') draft.answer = null; }
    else if (Array.isArray(draft.answer)) draft.answer = [...new Set(draft.answer)].filter(i => i < exercise.options.length).slice(0, ORDER_TYPES.has(exercise.type) ? exercise.answer.length : exercise.options.length);
...
export function responseReady(exercise, response) {
  if (exercise.type === 'typed_recall') return typeof response === 'string' && response.trim().length > 0 && response.length <= 500;
  ...
  return Array.isArray(response) && response.length > 0 && new Set(response).size === response.length
    ...
    && (!(ORDER_TYPES.has(exercise.type) || exercise.type==='match_pairs') || response.length === exercise.answer.length);
}
export const normalizeArabicAnswer = value => String(value || '').normalize('NFKC').replace(/[\p{M}\p{P}\u0640]/gu, '').replace(/[أإآٱ]/g, 'ا').replace(/\s+/g, ' ').trim();
export function gradeLiterature(exercise, response) {
  if (!responseReady(exercise, response)) return false;
  if (exercise.type === 'typed_recall') return [exercise.answer, ...(exercise.acceptedAnswers || [])].some(a => normalizeArabicAnswer(a) === normalizeArabicAnswer(response));
  if (CHOICE_TYPES.has(exercise.type)) return response === exercise.answer;
  const actual = ['chunk_sentence', 'clause_boundary'].includes(exercise.type) ? [...response].sort((a, b) => a - b) : response;
  return [exercise.answer, ...(exercise.acceptedAnswers || [])].some(expected => {
    const answer = ['chunk_sentence', 'clause_boundary'].includes(exercise.type) ? [...expected].sort((a, b) => a - b) : expected;
...
export function stepPassed(run, step) {
  return ['orientation', 'clean'].includes(step.type) ? !!run?.acknowledged[step.id]
    : step.assessment ? exerciseAttempts(run, step).length > 0
    : exerciseAttempts(run, step).some(a => gradeLiterature(step, a.answer));
}
export function runComplete(lesson, run) { return !!run && lessonSteps(lesson, run).every(step => stepPassed(run, step)); }
export function currentLiteratureStep(lesson, run) {
  const steps = lessonSteps(lesson, run);
...
export function exerciseFeedback(exercise, attempt) {
  ...
  if (exercise.type === 'match_pairs') {
    const wrong=exercise.answer.map((value,i)=>({value,i})).filter(({value,i})=>attempt.answer?.[i]!==value);
    return wrong.length?wrong.map(({value,i})=>`${exercise.focusWords?.[i]||exercise.stems[i]} → ${exercise.options[value]}`).join(' / '):'You matched each word to its meaning in this context. Later examples will test what stays familiar.';
  }
...
export function helpSteps(lesson, exercise) {
  if (exercise.support) return exercise.support;
  if (exercise.phase !== 'decode' && !exercise.useSource) return [{ kind: 'hint', text: exercise.hint || 'Read the Arabic in meaningful groups. Keep the participants and the order of events in view.' }];
  return [
...
export function optionOrder(run, exercise) {
  ...
  return ['chunk_sentence', 'clause_boundary'].includes(exercise.type) ? exercise.options.map((_, i) => i) : items;
}
...
export function updateLiteratureRun(lesson, run, action, value, now, attemptId) {
  ...
  if (!step) return next;
  const draft = next.drafts[step.id] || { answer: null, checking: false, revealed: false, conceptOpen: false, updatedAt: now, enteredAt: now };
  ...
  if (action === 'back') { next.position = Math.max(0, index - 1); }
    ...
    const decodeIndex = steps.findIndex(s => s.phase === 'decode');
    next.position = value === 'needs-support' && decodeIndex >= 0 ? decodeIndex : index + 1;
    if (value === 'needs-support' && decodeIndex >= 0) {
      const decode = steps[next.position];
      next.help[decode.id] = Math.max(1, next.help[decode.id] || 0);
    }
  }
  else if (action === 'help' && EXERCISE_TYPES.has(step.type)) next.help[step.id] = Math.min(helpSteps(lesson, step).length, (next.help[step.id] || 0) + 1);
  ...
  else if (action === 'text' && step.type === 'typed_recall' && !passed && !draft.checking) draft.answer = String(value || '').slice(0, 500);
    ...
    if (CHOICE_TYPES.has(step.type)) draft.answer = value;
      ...
      draft.answer = selected.includes(value) ? step.type==='match_pairs' ? selected.slice(0,selected.indexOf(value)) : selected.filter(v => v !== value) : [...selected, value];
    }
  }
  else if (action === 'clear' && !passed && !draft.checking) draft.answer = null;
  else if (action === 'check' && EXERCISE_TYPES.has(step.type) && !passed && !draft.checking && responseReady(step, draft.answer) && safeId(attemptId)) {
    next.attempts.push({ id: attemptId, exerciseId: step.id, at: now, answer: clone(draft.answer), hints: next.help[step.id] || 0, support: clone(next.help), responseMs: draft.enteredAt ? Math.max(0, now - draft.enteredAt) : 0 });
    draft.checking = true;
  }
  draft.updatedAt = now;
  ...
  next.updatedAt = now;
  const upcoming = steps[next.position];
  if (upcoming && !next.drafts[upcoming.id]) next.drafts[upcoming.id] = { answer: null, checking: false, revealed: false, conceptOpen: false, updatedAt: now, enteredAt: now };
  return next;
}
...
export function validateLiteratureLesson(lesson, knownTargets = null) {
  ...
  if (lesson.source?.kind!=='practice' && !lesson.source?.sentenceIds?.length && !lesson.source?.sentences?.length) fail('canonical source references required');
  if (lesson.source?.kind!=='practice' && (!lesson.chunks?.length || !lesson.support?.paraphrase || !lesson.support?.structure || !lesson.support?.vocabulary || !lesson.support?.explanation)) fail('complete progressive support required');
  ...
  for (const e of lesson.exercises || []) {
    ...
    if (e.type==='typed_recall' ? !gradeLiterature(e,e.answer) : !Array.isArray(e.options) || e.options.length < 2 || !e.options.every(o => typeof o === 'string' && o.trim()) || !gradeLiterature(e, e.answer)) fail(`invalid answer/options ${e.id}`);
    ...
    if (!['decode','assess'].includes(e.phase) && !e.hint) fail(`hint required: ${e.id}`);
    if (knownTargets && e.targets?.some(id => !knownTargets.has(id))) fail(`unknown language target: ${e.id}`);
  }
  if (lesson.flow) {
    if (new Set(lesson.flow).size !== lesson.flow.length || lesson.flow.some(id => id !== 'clean' && !lesson.exercises.some(e => e.id === id))) fail('flow must reference unique authored activities');
  }
  const transfer = lesson.exercises?.filter(e => e.phase === 'transfer') || [];
  if (!['review', 'checkpoint', 'cumulative', 'diagnostic', 'adaptive-review'].includes(lesson.kind) && transfer.length < 1) fail('requires novel transfer');
